...
- Market Data 股票“全部标的”列表改为每页 15 支并压缩行高；列表标题左侧新增全标的刷新图标按钮和微型进度条，可后台刷新全部标的近 6 个月日线、公司概况和股票财务数据；每次任务按标的检查本地 6 个月日线覆盖，已覆盖则跳过外部请求，并新增 15:30 自动刷新任务。
- Market Data 股票市场顶部每个宽基指数概览项支持点击展开独立 K 线卡片，展开时隐藏下方股票筛选、列表和详情；图表复用 Push Center 近 5 年宽基历史收盘与成交量，时间范围选项与股票 K 线一致，并在 overview 接口兼容新增 `indices[].daily_bars`。
