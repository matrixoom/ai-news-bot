...
- 股票市场自定义分组从浏览器 `localStorage` 迁移到 `.data/market_data.db`，详情星标加入/移出分组会持久化到后端本地库；手动刷新按钮改为刷新当前选中分组，15:30 自动任务改为只刷新自定义分组内标的最近一周行情。
- 股票市场标的市场类型统一归并沪深主板为 `沪市` / `深市`，前端下拉移除重复主板选项，仓储初始化与 `scripts/repair_stock_instrument_classification.py` 会修复旧库数据。
- 股票市场工作台调整“全部标的”区域：刷新按钮移动到顶部筛选栏右侧，列表支持本地自定义分组、详情星标加入分组、左右列表/详情宽度拖拽，并将最新价列移动到名称后方。
- 股市 K 线和宽基指数 K 线默认仅展示 MA5/MA10/MA20；MA20 保持实线，其余均线改为差异化虚线，降低长周期均线对默认视图的干扰。
- Market Data 股票“全部标的”每日刷新固定以任务当天作为每只标的的结束日期，默认从最后有效日线补到当天；若最后有效日前一周交易日不连续，则回退补刷最近一周数据。15:30 自动任务仍仅在 A 股开盘日触发，休市日不执行。
- Market Data 股票“全部标的”刷新新增近 20 年历史覆盖标记；历史模式会自动为既有完整数据补标并跳过已覆盖标的，当日模式只从已有日线标的的最后记录补到最近交易日，避免 ETF/LOF 空历史拖慢日常刷新。
- Market Data 股票“全部标的”刷新按股票、ETF、LOF 分池受控并发，并为 A 股 AkShare 主/腾讯端点设置 10 秒 timeout；前端刷新运行中降低状态轮询频率并保留详情旧数据，减轻后台写库时页面卡顿。
- Market Data 股票“全部标的”刷新运行中再次点击刷新按钮会弹出取消确认；确认后调用取消接口，将任务置为 `canceling/canceled`，停止尚未开始的补采并保留已写入日线。
- Market Data 股票“全部标的”历史刷新改为只按近 20 年或上市日至今检查日线首尾覆盖，窗口已覆盖时不再扫描中间零散缺口，避免老旧日期反复触发慢速上游失败。
- Market Data 股票“全部标的”刷新保留 3 个 worker 受控并发；当日刷新会定位最近收盘交易日，已有该交易日日线时跳过外部请求。
- Market Data 股票“全部标的”刷新页面时自动选中的首个标的不再累计访问次数，仅手动点击或搜索触发的详情读取会更新 `market_stock_instrument.access_count`。
- Market Data 股票“全部标的”手动刷新新增历史/当日模式选择；15:30 自动刷新改为当日模式，财务数据页新增独立刷新按钮并只刷新财务指标。
- Market Data 股票“全部标的”刷新改为只刷新基础 OHLCV 日线并计算均线，不再批量触发公司概况、估值、分红或财务指标接口；历史模式按近 20 年或上市日至今窗口刷新，定时刷新仍会先判断 A 股交易日。
- 股市 K 线和宽基指数 K 线图例统一放到图表顶部并保留 10px 间距，时间范围新增近 10 年和近 20 年；宽基 K 线页新增手动刷新按钮。
- 股票市场 K 线图移除 PE(TTM)、PB(MRQ)、股息率(TTM) 和总市值估值图例与曲线，估值数据改为仅在“基础行情观察”展示最新值。
- 股票市场标的分类修正：ETF 不再作为市场类型，基金类标的按沪市、深市或境外归类；`*ST/ST` 与退市标的从上市状态中拆分。
- 股票详情手动刷新现在会同时强制刷新日频估值与季度财务指标，避免已有旧财务记录时跳过 ROE、营收同比、净利润同比和资产负债率补采。
- Push Center 定时任务增加跨进程分钟槽位抢占，多个本地后端或热重载实例同时运行时，同一计划任务只会发送一封邮件。

### Added

- Market Data 商品页和贵金属页新增 `08:00`、`20:00`（`Asia/Shanghai`）后台自动刷新任务，分别同步 WTI/布伦特与黄金/白银/铜历史数据，同一进程内同一时间槽只执行一次。
- Market Data 股票“全部标的”列表新增访问次数排序，查看详情或刷新详情会累计标的访问次数，访问最多的标的优先展示。
- Market Data 股票详情“基础行情观察”右上角新增手动刷新按钮，可刷新最新行情并随日线数据重新计算海龟交易法指标。
- Market Data 股票详情海龟交易法指标旁新增问号提示，鼠标悬停可查看 TR、N、入场、离场和加仓信号的计算方法。
- Market Data 股票详情“基础行情观察”新增海龟交易法指标，基于日线高低收计算当日 TR、20 日平滑 N、20/55 日突破入场、10/20 日多头离场和 0.5N 加仓信号。
- Market Data 股票筛选新增 LOF 证券类型、境外市场类型和 ST/退市上市状态；基础行情观察新增当前选中区间涨幅，并随选中时间范围自动计算。
- Market Data 股票“全部标的”列表改为每页 15 支并压缩行高；列表标题左侧新增全标的刷新图标按钮和微型进度条，可后台刷新全部标的近 20 年日线和公司概况；每次任务按标的检查本地日线覆盖，已覆盖则跳过外部请求，并新增 15:30 自动刷新任务。
- Market Data 股票市场顶部每个宽基指数概览项支持点击展开独立 K 线卡片，展开时隐藏下方股票筛选、列表和详情；图表复用 Push Center 宽基历史收盘与成交量，时间范围选项与股票 K 线一致，并在 overview 接口兼容新增 `indices[].daily_bars`。
- Market Data 股票市场顶部概览复用 Push Center 默认宽基指数注册表，展示沪深 300、中证 500、中证 1000、上证综指、深证成指、创业板指、恒生科技指数和市场宽度；概览条压缩为单行紧凑网格。
- Market Data 股票详情的研究摘要侧栏支持向右折叠，折叠后 K 线图区域自动扩展，保留窄栏按钮用于重新展开。
- 全系统 UI 切换为 Trend Insight 研究工作台风格：新增亮暗语义令牌、公共面板/按钮/状态组件、统一 ECharts 主题、移动导航抽屉、显式导航编辑模式和专业占位态。
- 股票市场新增 `GET /api/frontend/modules/market-data/stocks/overview`，从市场指数历史库和股票日线分片返回上证指数、深证成指及真实涨跌家数；深证成指注册表、实时映射与近十年历史同步完成。
- 左侧导航栏新增 `AI Agent` 一级目录模块，并接入 `/ai-agent` 占位工作区与折叠态专属图标。
- 股票详情新增 PE(TTM)、PB(MRQ)、股息率(TTM)、总市值四项日频估值曲线，指标随 OHLCV 写入 `market_stock_daily_bar` 分片并在 K 线图右侧图例展示；财务页新增 ROE、营收同比、净利润同比、资产负债率四项季度指标，接口仅追加可空字段与序列以保持兼容。
- Market Data 股票市场页按可用高度自适应股票列表分页容量，概览 K 线与成交量图改为填满详情区，并新增不复权/前复权/后复权三段切换控件；同时压缩 K 线 tooltip、财务子页签及图表字号，隐藏成交量纵轴刻度数字并修复财务图表越界。
- 股票详情财务数据区将五项指标改为二级页签，财务时间范围与概览统一为月度到自定义选项且状态独立，季度/年度改为同一筛选栏右侧的分段选择器。
- 股票日线新增 210 个预建 SQLite 分片库：SH/SZ 分别按代码第 4、5 位拆分 100 库，BJ 按代码最后一位拆分 10 库；新增统一路由、schema 初始化和分片专项测试。
...
- Event Insight 新增 P10 发布硬化：新增 `GET /api/frontend/modules/event-insight/topics` 主题列表接口，主题溯源页改为先读取可选主题，移除固定 `topicId=1` 假设。
...
- Market Data 股票详情“基础行情观察”的海龟系统一/入场、离场信号改为下拉二选一展示，减少研究摘要并列信息密度。
- Market Data 股票页重构为市场摘要带、标的表格、分析画布和研究摘要四区；宏观、房地产、事件、推送和设置模块统一为紧凑面板、表格、筛选栏与对话框体系。
- LLM Provider 包改为按需导入具体 SDK，Web/CLI 启动不再提前加载未使用的 Gemini 实现，减少 IDE Debug 配合 Uvicorn reload 时的重复导入耗时和弃用警告。
- Market Data 股票市场页移除顶部“刷新基础标的”和自定义时间范围旁的 `R1` 重复入口，在“不复权”左侧统一提供行情刷新图标，并按当前选中的时间范围刷新数据。
- Market Data 股票详情页缩小概览时间跨度与复权控件，统一标的列表和右侧详情区底部高度，并为被截断的标的名称增加完整行信息悬停提示。
- Market Data 股票详情页移除顶部价格涨跌摘要与 K 线前的当日行情摘要，并压缩 K 线/成交量组合图高度，使成交量区域默认可在首屏底部显示。
- Market Data 股票详情页移除顶部冗余行情指标组和“加入自选”按钮；财务数据改为单指标 Tab 展示，并新增独立于行情概览的 `1/3/5/10 年 / 全部`时间范围控制。
- 修复股票行情在东方财富 AkShare 端点不可用时，腾讯回退数据因 `amount` 手数字段未转换而被误判为空的问题；腾讯成交量统一换算为股并保留新浪回退，股票详情与首次懒同步默认范围调整为近 1 个月。
- 股票日线分片从 `.data/market_stock_*.db` 调整为 `.data/market/{SH|SZ|BJ}/market_stock_*.db`；仓储启动时会自动搬迁旧扁平分片，并在新旧文件冲突时拒绝覆盖。
- `market_stock_daily_bar` 从 `.data/market_data.db` 迁移到 `market_stock_*.db`；仓储启动时分批幂等迁移旧表并校验，成功后收缩主库，股票列表改为按分片批量读取最新价，外部 API 契约保持不变。
- Market Data 股票列表首次切换到未缓存页时保留上一页数据与总数，避免加载期间分页器和页码短暂清空；股票详情恢复“概览 / 财务数据”页签，并将财务图表从概览迁移到独立财务页签。
- 修复 Market Data 股票市场页的列表高度、真实分页、筛选、折叠和自定义时间范围交互；左侧列表不再使用内部滚动条，详情区移除无功能的概览类标签。
- Market Data 股票市场页重做为截图式交易终端布局：顶部筛选栏、左侧标的表、右侧行情摘要、K 线、基础/行情侧栏和财报小图卡片；股票/ETF 标的同步改为分段容错并增加 AkShare 多端点回退，ETF 代理失败不再阻断 A 股标的入库；股票/ETF 日线同步失败改为短 warning 并保留旧数据，ETF 日线会回退新浪历史端点。
- Push Center 市场日报组合图画布扩展到全宽预览双栏的可用宽度，减少图表右侧大片留白，同时保留移动端 `max-width:100%` 缩放约束。
