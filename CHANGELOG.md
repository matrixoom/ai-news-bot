...
- Market Data 股票“全部标的”历史刷新改为按 A 股交易日历过滤本地日线缺口，休市窗口不再触发无效补采；批量补采使用 3 个 worker 受控并发提升刷新速度。
- Market Data 股票“全部标的”历史刷新会先检查本地日线缺口，仅补采明显缺失日期段；当日刷新会定位最近收盘交易日，已有该交易日日线时跳过外部请求。
...
- Market Data 股票“全部标的”刷新改为只刷新基础 OHLCV 日线并计算均线，不再批量触发公司概况、估值、分红或财务指标接口；历史模式按近 20 年窗口补采，不足 20 年的标的按有史以来可取数据返回，定时刷新仍会先判断 A 股交易日。
