...
- `.data/market/SH/market_stock_SH_00.db` 至 `market_stock_SH_99.db` 保存沪市日线
- `.data/market/SZ/market_stock_SZ_00.db` 至 `market_stock_SZ_99.db` 保存深市日线
- `.data/market/BJ/market_stock_BJ_0.db` 至 `market_stock_BJ_9.db` 保存北交所日线
- `src/services/stock_market_sharding.py` 是唯一分片路由和 schema 初始化入口
- `StockMarketRepository` 负责旧主库日线迁移、旧扁平分片搬迁、分片读写和按分片批量补充最新价
