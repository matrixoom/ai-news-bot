...
居民收入预期不作为第一版核心因子。该数据更偏问卷口径，连续可机读历史数据不稳定；后续若能确认稳定来源，可作为 `optional` 观察因子补充，不参与第一版核心批注评分。
...
- 信贷增量：居民新增贷款改善时加分，收缩时减分。
- 信贷存量增速：居民贷款增速上行时加分，持续下行时减分。
- 居民流动性：居民活期存款增速上行时加分，说明短期可动用资金改善；持续下行时减分。
- PPI：PPI 修复可作为企业利润和名义需求改善代理，但对居民购房需求权重低于贷款和居民流动性指标。
...
- 长期实际收入趋势。
- 居民部门信用扩张能力。
- 居民端流动性变化，第一版用居民活期存款增速代理。
- 房价相对信用扩张压力，第一版用房价指数与居民贷款增速构造简化指标。
...
| 字段 | 类型 | 中文说明 |
| --- | --- | --- |
| `factor_code` | `TEXT NOT NULL` | 因子编码，例如 `gdp_nominal`、`gdp_real`、`gdp_gap`、`ppi`、`household_new_loans`、`household_loan_growth`、`household_demand_deposit_growth`。 |
...
| `factor_label` | `TEXT NOT NULL` | 因子中文名称，例如 `居民新增贷款`、`居民贷款增速`、`居民活期存款增速`。 |
...
| 字段 | 类型 | 中文说明 |
| --- | --- | --- |
| `source_key` | `TEXT NOT NULL` | 数据来源唯一标识，例如 `nbs_70_city_price`、`pbc_financial_statistics`、`pbc_loan_report`。 |
...
- 因子图展示 GDP、PPI、居民新增贷款、居民贷款增速、居民活期存款增速等宏观因子的历史走势。
