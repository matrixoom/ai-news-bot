...
用途：启动“全部标的”后台刷新任务。服务会逐只刷新当前本地上市和 ST 状态股票、ETF、LOF 的近 6 个月日线行情和公司概况；股票额外刷新财务指标，ETF/LOF 不拉取公司财报。该任务异步执行，手动刷新和自动定时刷新共用同一任务状态。服务按上海本地日期做每日限次，同一天最多实际执行一次；重复点击会返回正在运行的任务或 `skipped` 状态。
