...
- 就业：`unemployment_insurance_fund_expense` 图表读取 `macro_unemployment_insurance_fund_expense` 年度事实表；当前页面先提供样例年度序列，真实近 30 年数据回填后再切换同步来源。
