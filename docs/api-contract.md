...
股票市场服务启动后会在后台检查上海时间 `15:30` 槽位，并先通过 A 股交易日历判断当天是否开盘；周末和节假日休市日不会触发 `scheduled` 全标的刷新。自动任务和手动任务共用最近一次完成状态文件 `.data/stock_market_refresh_state.json`，不再按自然日限制启动；定时任务使用 `mode=today`，只从已有本地日线标的的最后有效日期补到当天，最近一周存在交易日缺口时回退补刷最近一周，不触发公司概况、估值、分红或财务指标接口。
